    ...
    <div className="relative min-h-[60vh]" dir="rtl">
      ...
      <div className={`mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 ${VOTING_ENABLED ? 'lg:ps-28' : ''}`}>
        ...
        <div className="mb-6 flex flex-wrap gap-2 rounded-2xl border border-border bg-white p-3">
          ...
          {(
            [
              ['all', fa.events.dateAll],
              ['today', fa.events.dateToday],
              ['week', fa.events.dateWeek],
              ['month', fa.events.dateMonth],
            ] as const
          ).map(([id, label]) => (
            <button
              key={id}
          ...
          >
            <option value="all">{fa.events.genderAll}</option>
            <option value="everyone">{fa.events.genderEveryone}</option>
        ...
        <section className="mt-12">
          <h2 className="mb-4 text-xl font-black text-foreground">{fa.events.friendsGoing}</h2>
          {!userId ? (
            <p className="rounded-2xl border border-border bg-white px-4 py-6 text-sm text-muted">
              {fa.events.friendsLoginHint}{' '}
