...
function timeAgo(dateStr: string) {
  ...
  if (minutes < 1) return fa.happeningNow.createdJustNow;
  if (minutes < 60) return `${minutes} ${fa.happeningNow.createdMinutesAgo}`;
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return `${hours} ${fa.happeningNow.createdHoursAgo}`;
  const days = Math.floor(hours / 24);
  return `${days} ${fa.happeningNow.createdDaysAgo}`;
}
  ...
  return (
    <div className="py-12 bg-background border-b border-gray-100 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6 flex items-center gap-3">
        <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">{fa.happeningNow.title}</h2>
            ...
            >
              ...
              {(evt.host?.is_verified || true) && (
                <div 
                  ...
                  title={fa.happeningNow.verifiedHost}
                >
              ...
              <div className="flex items-center justify-between mb-3">
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold ${color.bg} ${color.text}`}>
                  <span>{getEmojiForCategory(evt.category?.name)}</span> {categoryFa(evt.category?.name)}
                </span>
              </div>
